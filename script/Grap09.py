...
df['age_vehicle'] = current_year - df['Model']

# Calcular la tasa de depreciación
df['depreciation_rate'] = 1 - (df['Current Price'] / df['Initial Price']) ** (1 / df['age_vehicle'])


# Calcular la depreciación usando el método de saldo decreciente (Declining Balance Method)
df['Annual Depreciation'] = df['depreciation_rate'] * df['Current Price']
...
plt.savefig('figures\Fig09.png', bbox_inches='tight', dpi=300)
plt.savefig('figures\Fig09.pdf', bbox_inches='tight', dpi=300)

# Exportar la tabla pivote a un archivo CSV
csv_file_path = 'mnt\data\Mod_2_DF.csv'
