...
# Usar el punto de intersección (X = 21.95) para ajustar la depreciación
X_intersect = 21.95
...
df['Quality'] = 100 - (df['Accumulated kilometers'] / 1000)

# Calcular la depreciación anual ajustada por la calidad percibida y el punto de intersección
df['Annual Depreciation'] = (
    (df['Initial Price'] - df['Current Price']) / df['life_years']
) * (df['Quality'] / X_intersect)
...
plt.savefig('figures\Fig12.png', bbox_inches='tight', dpi=300)
plt.savefig('figures\Fig12.pdf', bbox_inches='tight', dpi=300)

# Exportar la tabla pivote a un archivo CSV
csv_file_path = 'mnt\data\Mod_4_DF.csv'
