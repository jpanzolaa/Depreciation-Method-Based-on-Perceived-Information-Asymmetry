...
df['life_years'] = current_year - df['Model']

# Utilizar Current Price como el valor residual
df['Residual Value'] = df['Current Price']

# Calcular la suma de los dígitos de la vida útil
df['sum_of_years_digits'] = df['life_years'].apply(lambda x: sum(range(1, x + 1)))

# Calcular la depreciación para cada vehículo en el primer año utilizando SYD
df['Annual Depreciation'] = ((df['life_years']) / df['sum_of_years_digits']) * (df['Initial Price'] - df['Residual Value'])
...
plt.savefig('figures\Fig10.png', bbox_inches='tight', dpi=300)
plt.savefig('figures\Fig10.pdf', bbox_inches='tight', dpi=300)

# Exportar la tabla pivote a un archivo CSV
csv_file_path = 'mnt\data\Mod_3_DF.csv'
